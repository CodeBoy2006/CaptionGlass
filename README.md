...
需要 **Android 10+、arm64 设备，以及支持 Vulkan 1.2 和所需算子的 GPU**。模型体积和运行内存需求差异较大，下载前可在型号详情查看占用；设备支持 Vulkan 不等于所有模型都能流畅运行。
...
- **播放应用需要允许音频捕获。** 并非所有应用或内容都可用；没有声音时会提示，静音本身不能说明原因。
- **字幕质量与等待时间取决于语音、模型和设备。** 错词、漏句及长句等待仍可能发生；积压或超时会明确标为未译，不自动切换云端。
- **记录只保留在本次会话的内存中。** 当前没有持久保存、字幕导出、术语表或跨进程断点续传。

后续重点是记录与导出，以及伴随视频播放的 30–60 分钟、多机型和自然语音验证。[验收记录](docs/validation.md)区分历史测试结果、已知问题与待达成目标。
...
项目尚未选择公开分发的软件许可证。第三方运行库和模型遵循各自许可证，详见[第三方声明](third_party/NOTICE.md)；公开分发前仍需完成传递依赖审计。
