...
| **随时回看，继续跟随** | 滚动查看本次会话最近 1000 条记录，回看时保持阅读位置，回到底部继续跟随新字幕。 |
| **记录导出** | 在记录页选择 TXT、JSON 或 CSV，通过系统文件选择器保存原文、译文、会话时间与处理状态。 |
| **按画面选择显示** | 滚动、两段、一段三种方式；黑曜底板或描边外观，支持系统字号缩放与深浅主题。 |
| **空闲自动收起** | 15 秒无字幕更新或交互后，悬浮框清空并折叠为胶囊；新字幕到达时展开，记录页内容保留。 |
...
1. **安装应用。** 按[构建说明](docs/architecture.md#6-构建与开发)生成并安装 APK；调试用 Debug，较小的分发包用 Release（需自行签名）。
...
5. **结束观看与导出。** 在应用或前台通知中停止；进入「记录 → 导出」，选择 TXT、JSON 或 CSV，再选择保存位置。已显示的记录保留在内存中，开启新会话或进程退出会清空，需保留时请提前导出。

导出的是选择格式时记录页保留的快照，最多 1000 条，按片段顺序排列；运行中也可导出，处理中或失败时留下的译文会标为未完成。时间从本次音频采集开始计算，不是源视频时间轴。

TXT 适合阅读；JSON 包含语言、会话与片段身份、毫秒时间和处理状态；CSV 使用带 BOM 的 UTF-8，可导入表格软件。JSON／CSV 的 `status` 为 `TRANSLATED`、`PENDING` 或未翻译原因（`BACKLOG`、`FAILED`、`TIMED_OUT`、`STOPPED`、`SUPERSEDED`），只有 `TRANSLATED` 表示完整译文。CSV 对可能被表格识别为公式的文本加单引号前缀；需要原始文本时使用 TXT 或 JSON。
