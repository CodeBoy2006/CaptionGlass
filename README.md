...
需要 **Android 10+、arm64 设备**。翻译默认使用 GPU (Vulkan)，要求 Vulkan 1.2 及所需算子；也可在「设置 → 翻译处理器」选择 CPU 或实验性的 NPU (Hexagon)。Hexagon 仅适用于提供兼容驱动的骁龙设备，并需选择兼容量化模型（目前提供 Hy-MT2 Q8_0）；联发科 NPU 不在此后端支持范围。模型体积和运行内存需求差异较大，下载前可在型号详情查看占用；设备支持 Vulkan 不等于所有模型都能流畅运行。
